...
- (void)initialization
{
    self.dotViewClass           = [YCAnimatedDotView class];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(YCPageControl:didSelectPageAtIndex:)]) {
            [self.delegate YCPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView {
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[YCAnimatedDotView class]] && self.dotColor) {
            ((YCAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    }else {
...
{
    if (self.dotViewClass) {
        YCAbstractDotView *abstractDotView = (YCAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [YCAbstractDotView class]);
        }
