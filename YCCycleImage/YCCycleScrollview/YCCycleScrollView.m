...
NSString *const ID = @"YCCycleScrollViewCell";
...
- (void)initialization {
    _pageControlAliment = YCCycleScrollViewPageContolAlimentCenter;
    ...
    _pageControlStyle = YCCycleScrollViewPageContolStyleClassic;
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame imageNamesGroup:(NSArray *)imageNamesGroup {
    YCCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame delegate:(id<YCCycleScrollViewDelegate>)delegate placeholderImage:(UIImage *)placeholderImage
{
    YCCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)setupMainView {
    ...
    [mainView registerClass:[YCCollectionViewCell class] forCellWithReuseIdentifier:ID];
...
- (void)setDelegate:(id<YCCycleScrollViewDelegate>)delegate {
    _delegate = delegate;
...
- (void)setPageControlDotSize:(CGSize)pageControlDotSize {
    ...
    if ([self.pageControl isKindOfClass:[YCPageControl class]]) {
        YCPageControl *pageContol = (YCPageControl *)_pageControl;
...
- (void)setCurrentPageDotColor:(UIColor *)currentPageDotColor {
    ...
    if ([self.pageControl isKindOfClass:[YCPageControl class]]) {
        YCPageControl *pageControl = (YCPageControl *)_pageControl;
...
- (void)setCurrentPageDotImage:(UIImage *)currentPageDotImage
{
    ...
    if (self.pageControlStyle != YCCycleScrollViewPageContolStyleAnimated) {
        self.pageControlStyle = YCCycleScrollViewPageContolStyleAnimated;
    }
...
- (void)setPageControlStyle:(YCCycleScrollViewPageContolStyle)pageControlStyle
{
...
- (void)setupPageControl {
    ...
    switch (self.pageControlStyle) {
        case YCCycleScrollViewPageContolStyleAnimated:
            {
                YCPageControl *pageControl = [[YCPageControl alloc] init];
            ...
            break;
        case YCCycleScrollViewPageContolStyleClassic:
        {
...
- (int)currentIndex {
    if (_mainView.yc_width == 0 || _mainView.yc_height == 0) {
        return 0;
    }
    int index = 0;
...
- (void)layoutSubviews {
    ...
    CGFloat x = (self.yc_width - size.width) * 0.5;
    if (self.pageControlAliment == YCCycleScrollViewPageContolAlimentRight) {
        x = self.mainView.yc_width - size.width - 10;
    }
    CGFloat y = self.mainView.yc_height - size.height - 10;
...
// 当视图即将加入父视图时 或 当视图即将从父视图移除时调用,解决当父View释放时，当前视图因为被Timer强引用而不能释放的问题
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    YCCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
  
    long itemIndex = [self pageControlIndexWithCurrentCellIndex:indexPath.item];
    if ([self.delegate respondsToSelector:@selector(setupCustomCell:forIndex:cycleScrollView:)] && [self.delegate customCollectionViewCellClassForCycleScrollView:self]) {
